...
def eval_hessian(point1,point2):
	'''Returns 2x2 numpy array of the symbolic hessian function evaluated at (point1,point2)'''

	p1_1 = hessian[0][0].subs([(x1,point1),(x2,point2)]).evalf()
	p1_2 = hessian[0][1].subs([(x1,point1),(x2,point2)]).evalf()
	p2_1 = hessian[1][0].subs([(x1,point1),(x2,point2)]).evalf()
	p2_2 = hessian[0][1].subs([(x1,point1),(x2,point2)]).evalf()
...
for i in range(len(x_k_list)): 
	ax.scatter(x_k_list[i][0],x_k_list[i][1],f_list[i],c= 'black',marker = '^',s = 25)# place down x_k markers
