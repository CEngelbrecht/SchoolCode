...
def eval_hessian(x_k):
	'''Returns 2x2 numpy array of the symbolic hessian function evaluated at x_k'''
...
while np.sqrt(float(gradient[0]**2 + gradient[1]**2)) > 1E-12:
	...
	print("alpha_k = {}, x_k = {}, function_value = {}, p_k = {}".format(alpha_k,x_k,function_value,p_k)) 
	#print("Gradient = {}, Hessian = {}".format(gradient,evaluated_hessian))
	x_k_list.append(x_k)
	f_list.append(function_value)
	alpha_list.append(alpha)

	
	x_k = x_k + alpha_k*p_k #Iterating with step size gained from backtracking 
	function_value = eval_function(x_k) #Checking current value of the function
...
plt.suptitle('(-1.2,1.0) Newton with Backtracking Line Search')
